...
{
	string data;
	if (MedSerialize::read_file_into_string(fname, data) < 0)
		return -1;
...
int SerializableObject::init_param_from_file(string file_str, string &param)
{
	...
	if (upper_cp.compare(0, 5, "FILE:") == 0)
	{
		...
		if (MedSerialize::read_file_into_string(fname, param) < 0)
			return -1;
	}
	...
	if (upper_cp.compare(0, 9, "LIST_REL:") == 0)
	{
		...
		if (MedSerialize::read_list_into_string(fname, param) < 0)
			return -1;
	}
